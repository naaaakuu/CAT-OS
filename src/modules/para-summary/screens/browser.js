/**
 * browser.js — the Para Summary library as a summary JOURNEY:
 ...
 * one recommended next paragraph carrying the lamp. Tiers recommend;
 * nothing locks (the same principle as the RC and PJ journeys).
 */

import { listPSItems } from '../../../core/content-loader/loader.js';
import { groupByTier, recommendNextPS, tierInfo } from '../logic/tiers.js';
import { latestByItem } from '../logic/store.js';
import { escapeHTML } from '../../../core/utils/format.js';

export async function renderPSBrowser(outlet, { storage }) {
  outlet.innerHTML = `
    <section class="screen">
      <header class="journey-head">
        <p class="screen__eyebrow">Practice · Para Summary</p>
        <h1>Your summary journey</h1>
      </header>
      <div id="ps-list" aria-busy="true">
        <div class="skeleton skeleton--line" style="width: 40%"></div>
        <div class="skeleton"></div><div class="skeleton"></div><div class="skeleton"></div>
      </div>
    </section>
  `;
  const list = outlet.querySelector('#ps-list');
  const done = () => list.removeAttribute('aria-busy');

  let items;
  let latest;
  try {
    [items, latest] = await Promise.all([listPSItems(), latestByItem(storage)]);
  } catch (err) {
    list.innerHTML = `<div class="card"><p>${escapeHTML(err.message)}</p>
      <p class="muted">If you are offline, open this screen once online so
      paragraphs can be cached.</p></div>`;
    done();
    return;
  }
  ...
  if (items.length === 0) {
    list.innerHTML = `<div class="empty">
      ...
      to <code>content/para-summary/</code> and register them.</p>
    </div>`;
    return;
  }
  ...
  const next = recommendNextPS(items, solvedIds, triedIds);
  const groups = groupByTier(items);

  const progressHTML = `
    <p class="journey-head__sub">${solvedIds.size === 0
      ? `${items.length} paragraphs, from first steps to Premium`
      : `${solvedIds.size} of ${items.length} paragraphs summarised`}</p>
    ...
    <p class="hint" style="margin: 0 0 var(--space-3)">
      <a href="#/ps/about">How this journey works</a></p>`;
  ...
  const itemHTML = (item) => {
    ...
    const status = a && a.is_correct !== null
      ...
      : isNext
        ...
        : '';
    return `
      <a class="list-item ${isNext ? 'list-item--next' : ''}" href="#/ps/session/${item.id}"
        ...
        <div class="list-item__meta">
          ...
          <span>~${item.estimated_time_min} min · one paragraph</span>
          ${status}
        </div>
        ${isNext && next.reason
      ...
      ${a && a.is_correct !== null ? `
        <p class="row__hint" style="margin: calc(-1 * var(--space-2)) var(--space-2) var(--space-3)">
          <a href="#/ps/learn/${item.id}">Learning Page</a>
        </p>` : ''}
    `;
  };
    ...
    ${groups.map(({ tier, items: group }) => {
      ...
      return `
        <div class="stage-head">
          ...
          <div class="rule"></div>
          <span class="stage-head__count">${solved} of ${group.length} matched</span>
        </div>
        ...
        <p class="stage-desc" style="margin-top: calc(-1 * var(--space-2))">
          <a href="#/ps/session/${escapeHTML(tier)}">
