...
_Last updated: 2026-07-10 — Para Jumbles module complete. App version 0.10.0._
...
| Service worker / offline caching | **shipped** | Two caches: shell (v11) + content (v7), cache-first; content cached on first use as the library grows |
...
| Para Summary / Odd One Out / Vocabulary | designed | V1.x — copy the module pattern; Odd One Out reuses `<cat-jumble-board>` |
...
| System | State | Notes |
|---|---|---|
| Registry (`content/index.json`) | **shipped** | 32 RC + 19 PJ items; type-aware agreement with files (checked by tools/verify.mjs); rebuilt from files by a scripted mirror |
...
| Content library | **shipped** | 32 RC passages (136 questions; foundation→elite, all 12 genres) + **19 Para Jumbles (all 8 tiers Beginner→Premium, difficulty_numeric 1→10, paragraph-first then scrambled, adversarial from Medium up)** |
| Verification tool (`tools/verify.mjs`) | **shipped** | Reuses the app's own validators (RC + PJ); type-aware registry agreement, module-graph + precache-coverage (every schema version), journey-progression, backup round-trip, and a full PJ engine/voice/DNA/lesson dry run; mentor-voice lint covers both mentors; runs on Windows (pathToFileURL fix); run before every release |
| Content schemas for PS/OOO/Vocab | designed | Author with each module |
